...
const FundRiserForm = () => {
  const [isOpenCreateNewProjectForm, setIsOpenCreateNewProjectForm] =
    useState(false);
  ...
  return (
    <div className='flex flex-col justify-center items-center gap-3'>
      <h1 className='font-sans font-bold text-xl'>
        Mulai Fundraising bersama Crowdfunding
      </h1>
      <p>Isi formulir di bawah ini dengan detail proyek Anda dan mulailah perjalanan penggalangan dana Anda. Pastikan untuk menyajikan informasi yang jelas dan inspiratif.</p>
      <button
        type='button'
        className='inline-block px-6 py-2.5 bg-emerald-500
        text-white font-medium text-xs leading-tight uppercase
        rounded-md shadow-md hover:bg-emerald-700'
        onClick={() =>
          setIsOpenCreateNewProjectForm(!isOpenCreateNewProjectForm)
        }
      >
        {isOpenCreateNewProjectForm ? 'Close Form' : 'Add Project'}
      </button>
      {isOpenCreateNewProjectForm && (
        <form onSubmit={(e) => riseFund(e)}>
          ...
          <div className='form-control my-1'>
            <label className='text-sm text-gray-700'>
              Jumlah kontribusi yang ditargetkan (ETH) :
            </label>
            <input
              type='number'
              placeholder='Type here'
